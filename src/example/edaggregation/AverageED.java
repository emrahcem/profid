...
implements CDProtocol, EDProtocol {
	...
	{
		//System.err.println(node.getID()+" nextCycle() is called");
		Linkable linkable = 
		...
		if (linkable.degree() > 0)
		{
			...
			System.out.println("nextCycle:"+node.getID()+" will send "+value);
	...
	public void processEvent( Node node, int pid, Object event ) {
		System.err.println(node.getID()+" processEvent() is called");	
		AverageMessage aem = (AverageMessage)event;


		if( aem.sender!=null ){
			System.out.println("processEvent:"+node.getID()+" will send "+value);
			((Transport)node.getProtocol(FastConfig.getTransport(pid))).
			send(
					node,
					aem.sender,
		...
		System.out.println("before avg:"+node.getID()+":"+value);
		value = (value + aem.value) / 2;
		System.out.println("after avg:"+node.getID()+":"+value);
	}
