...
*/
public class CustomGraphIO{
private CustomGraphIO() {}
...
public static void writeNeighborList( Graph g, PrintStream out ) {
	...
	for(int i=0; i<g.size(); ++i)
	{
		val+=g.getNeighbours(i).size();
		//added to correct node ids in each experiment (new experiment does not start with node id 0)
		out.print(Network.size()*Simulator.experimentNo+i+" ");
...
public static void writeDOT( Graph g, PrintStream out ) {
	...
	for(int i=0; i<g.size(); ++i)
	{
		...
		while(it.hasNext())
		{
			final int j = it.next();
			if(g.directed())
				out.println(Network.size()*Simulator.experimentNo+i+" -> "+j+";");
			else if( i<=j )
				out.println(Network.size()*Simulator.experimentNo+i+" -- "+j+";");
		}
	}
...
public static void writeGML( Graph g, PrintStream out ) {
	...
	for(int i=0; i<g.size(); ++i)
		out.println("node [ id "+(Network.size()*Simulator.experimentNo+i)+"  label \""+(Network.size()*Simulator.experimentNo+i)+"\" ]");
	...
	for(int i=0; i<g.size(); ++i)
	{
		...
		{
			out.println(
					"edge [ source "+(Network.size()*Simulator.experimentNo+i)+" target "+it.next()+" ]");
		}
	}
